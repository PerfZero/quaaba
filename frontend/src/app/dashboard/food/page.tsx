...
export default function FoodPage() {
  ...
  const fetchFood = async () => {
    try {
      ...
      message.error("Ошибка при загрузке питания");
    }
  };
  ...
  const handleCreateFood = async (values: { name: string; status: boolean }) => {
    setSaving(true);
    try {
      const response = await fetch(`${API_URL}/food`, {
        method: "POST",
      ...
      if (response.ok) {
        message.success("Питание успешно создано");
        setAddDrawerOpen(false);
        form.resetFields();
        fetchFood();
      } else {
        message.error(result.message || "Ошибка при создании питания");
      }
    } catch (error) {
      console.error("Error creating food:", error);
      message.error("Ошибка при создании питания");
    } finally {
      setSaving(false);
    }
  };

  const handleUpdateFood = async (values: { name: string; status: boolean }) => {
    if (!selectedFood) return;

    setSaving(true);
    try {
      const response = await fetch(`${API_URL}/food/${selectedFood.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        message.success("Питание успешно обновлено");
        setIsEditing(false);

        const foodResponse = await fetch(`${API_URL}/food/${selectedFood.id}`);
        const foodResult = await foodResponse.json();
        if (foodResult.data) {
          const formattedFood = {
            key: foodResult.data.id.toString(),
            id: foodResult.data.id,
            name: foodResult.data.name,
            status:
              foodResult.data.status === "active"
                ? "Активный"
                : "Неактивный",
            createdAt: foodResult.data.createdAt,
            updatedAt: foodResult.data.updatedAt,
          };
        ...
        message.error(result.message || "Ошибка при обновлении питания");
      }
    } catch (error) {
      console.error("Error updating food:", error);
      message.error("Ошибка при обновлении питания");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteFood = async (foodId: number) => {
    try {
      ...
      if (response.ok) {
        message.success("Питание успешно удалено");
        fetchFood();
        ...
        message.error(result.message || "Ошибка при удалении питания");
      }
    } catch (error) {
      console.error("Error deleting food:", error);
      message.error("Ошибка при удалении питания");
    }
  };

  const handleDeleteSelected = async () => {
    if (selectedRowKeys.length === 0) {
      message.warning("Выберите питание для удаления");
      return;
    }

    Modal.confirm({
      title: "Удалить выбранное питание?",
      ...
      onOk: async () => {
        try {
          const deletePromises = selectedRowKeys.map((key) =>
            fetch(`${API_URL}/food/${key}`, { method: "DELETE" }),
          );
        ...
        extra={
          <Space>
              ...
              <Button
                onClick={() => {
                  ...
                  editForm.setFieldsValue({
                    ...
                    status: selectedFood?.status === "Активный",
                  });
                }}
              >
                Изменить
            ...
            {!isEditing && (
              <Popconfirm
                title="Удалить питание?"
                description={`Вы уверены, что хотите удалить ${selectedFood?.name}?`}
                onConfirm={() => {
                  if (selectedFood) {
                ...
                <div>
                  ...
                  <Flex vertical gap={16}>
                    <div>
                      ...
                      <Typography.Text>{selectedFood.name}</Typography.Text>
                    </div>
                    <div>
                      <Typography.Text
                        type="secondary"
                ...
                <div>
                  ...
                  <Flex vertical gap={12}>
                    ...
                    <div>
                      ...
                      <Typography.Text>
                        {formatDate(selectedFood.updatedAt)}
                      </Typography.Text>
                    </div>
                  </Flex>
                </div>
